...
def model_health_report(
    ...
    weeks_per_bg = weekly_full.groupby("Barangay_standardized")["WeekStart"].nunique().unique()

    checks = {
        "Data totals match": abs(total_cases_raw - total_cases_weekly) < 10,
        "Constant weeks per barangay": len(weeks_per_bg) == 1,
        "No null barangays": df["Barangay_standardized"].isna().sum() == 0,
        ...
        "Barangay sum = city total (test+future)": diff < 1e-6,
    }

    report = pd.DataFrame(
        {"Check": list(checks.keys()), "Status": ["✅ PASS" if v else "❌ FAIL" for v in checks.values()]}
    )
    try:
